...
const Sidebar = () => {
  ...
  return (
    <div className="flex items-end justify-end h-screen ">
      <button
        className="fixed flex items-center justify-center w-10 h-10 text-4xl text-white duration-300 bg-teal-800 rounded-full lg:hidden z-90 bottom-10 right-8 drop-shadow-lg hover:bg-teal-800"
              ...
              {Menu.subMenus && subMenuOpen && open && (
                <ul>
                  {Menu.subMenus.map((subMenuItem, idx) => (
                    <li
                      key={idx}
                      className="flex px-5 py-1 text-sm text-center text-gray-200 cursor-pointer"
                    >
                      {subMenuItem.title}
                    </li>
                  ))}
